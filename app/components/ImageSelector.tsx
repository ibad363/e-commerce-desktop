    ...
    return (
        <>
            ...
            <div className='flex flex-row lg:flex-col justify-between gap-2 lg:gap-8'>
                {images.map((key: any, i: number) => (
                    <div
                        ...
                        onClick={() => { setActive(i); }}>
                        <Image src={urlFor(key).url()} alt='Product Image' width={120} height={120} className='object-contain'></Image>
                    </div>
                ))}
            ...
            <div className='bg-[#FBEBB5] max-w-[750px] w-full h-[280px] xs:h-[400px] lg:h-auto flex justify-center items-center rounded-xl'>
                <Image src={urlFor(images[active]).url()} width={450} height={320} alt='Product Image' className='' />
            </div>
        </>
    );
};
